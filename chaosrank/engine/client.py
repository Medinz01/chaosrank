"""
chaosrank/engine/client.py

HTTP client for the private ChaosRank engine API.

Drop-in replacement for the old direct function calls:
  Old: blast = compute_blast_radius(G)
       ranked = rank_services(blast, incidents, alpha=0.6, beta=0.4)

  New: ranked = client.rank(G, incidents, config={...})
...
Usage:
    ...
    ranked = client.rank(G, service_incidents)

    # Adaptive:
    ranked = client.adaptive_rank(G, service_incidents)
    client.record_outcome(ranked[0], outcome="WEAKNESS_CONFIRMED")

    # Multi-agent merge:
    merged_graph_payload = client.merge_snapshots([snap_a, snap_b])

    # Federation:
    ranked = client.federation_rank(domains, inter_domain_edges)
"""
