"""
parser/otlp_proto.py — OTel OTLP protobuf binary trace parser

Parses binary-encoded OTLP protobuf trace exports produced by:
  - OTel Collector (otlp exporter, protobuf encoding)
  - Any SDK exporting to OTLP/gRPC (binary wire format)

Requires the optional 'protobuf' extra:
...
Entry point:
    ...
    edges = parse_otlp_proto(Path("traces.pb"))

CLI flag:
    chaosrank rank --traces traces.pb --otlp-format protobuf

Format detection:
    The --otlp-format protobuf flag is authoritative.
    A secondary magic-bytes check runs on startup and warns (does not error)
    if the file looks like JSON — in case the user passed the wrong flag.

Streaming:
    Binary protobuf is not newline-delimited; chunked streaming requires a
    varint-length-prefixed framing parser. For v0.4 this is not implemented.
    Files > 100 MB are parsed fully and a warning is emitted.
    Track issue #16 for protobuf streaming support.

Proto schema:
    opentelemetry.proto.trace.v1.trace_pb2.TracesData
        .resource_spans[*]
            .resource.attributes[*]   <- service.name lives here
            .scope_spans[*].spans[*]  <- span data
"""
...
# ---------------------------------------------------------------------------
# Optional import — clear error if missing
# ---------------------------------------------------------------------------
