...
Pulls from two Datadog APIs:
  - Events API v2   : monitor alert transitions (general alerts, warn/critical)
  - Incidents API   : formal incident records (SEV-1 / SEV-2 = P1 / P2)

Both are merged and deduplicated before returning.
request_volume is not populated here — left as None.
#15 (Prometheus adapter) will backfill request_volume globally.

Usage:
    adapter = DatadogIncidentAdapter(
        api_key="DD-API-KEY",
        app_key="DD-APPLICATION-KEY",
    )
    incidents = adapter.fetch(window_days=30)
"""
...
# ---------------------------------------------------------------------------
# Datadog-specific severity mappings
# ---------------------------------------------------------------------------

# Formal incident severity (SEV-1 .. SEV-5).
# Not covered by base normalize_severity — kept private here.
...
def _extract_service(tags: list[str]) -> Optional[str]:
...
class DatadogIncidentAdapter(IncidentAdapter):
    ...
    # ------------------------------------------------------------------
    # Events API v2  (monitor alert transitions)
    # ------------------------------------------------------------------
    ...
    def _parse_monitor_event(self, event: dict) -> Optional[Incident]:
    ...
    # ------------------------------------------------------------------
    # Incidents API  (formal P1/P2 incident records)
    # ------------------------------------------------------------------
